...
function AnimateBorder(el, options = {}) {
  ...
  function sync() {
    ...
    track.setAttribute('x', 0 - bLeft / 2); // Track is centered on the middle of the border, so half the border width is outside the element bounds.
    track.setAttribute('y', 0 - bTop / 2); // Track is centered on the middle of the border, so half the border width is outside the element bounds.
